...
class Hand:
    ...
    def ikSolver(self, robot, cspace, obj_pt, transfer):
        ...
        if not transfer:
            ...
            solution = self.goal_rotation_transfer(robot, cspace, obj_pt)
                ...
                for k in range(k_old, 2 * n):
                    ...
                    for l in range(l_old, sens):
                        for m in range(m_old, sens):
                            ...
                            if res is not None:
                                goal_config_transit = [i+1, j+1, k+1, l+1, 0, m+1]
                                return res
...
class GLPickAndPlacePlugin(GLPluginInterface):
    ...
    def keyboardfunc(self, key, x, y):
        ...
        if key == 's':
            ...
            vis.animate(("world", self.collis_object.getName()), self.objectPath, endBehavior='halt')
        return False

    def planTask(self, dest):
        global goal_config_transit
        global goal_rotation_transfer
        goal_config_transit = None
        goal_rotation_transfer = None
    ...
    def transfer_uti(self):
        ...
        self.Tgoal_col = graspedObjectTransform(self.robot, self.hand, self.transferPath[0],
                                                self.Tstart_col,
                                                self.transferPath[-1])
        #shift = vectorops.sub(self.Tgoal_col[1], self.Tstart[1])
        #self.Tgoal_col = (self.Tgoal_col[0], vectorops.add(self.Tgoal_col[1], shift))
...
def planTransit(world, objectIndex, hand):
    ...
    solution = hand.ikSolver(robot, cspace, obj.getTransform()[1], False)
