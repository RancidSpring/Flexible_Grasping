...
class Hand:
    ...
    def ikSolver(self, robot, cspace, obj_pt, transfer):
        """Returns an IK solver that places the hand center at obj_pt with
        the palm oriented along obj_axis"""
        numAngles = 6
        if not transfer:
            grasp, grasparm = self.goal_rotation_transit(robot, cspace, obj_pt, numAngles)
        else:
            grasp, grasparm = self.goal_rotation_transfer(robot, cspace, obj_pt)
        return grasp, grasparm

    def goal_rotation_transit(self, robot, cspace, obj_pt, n):
        """
        Rotates the gripper and tries grasping from each side specified by 3 angles
                ...
                for j in range(2 * n):
                    ...
                    for k in range(2 * n):
                        ...
                        rotObjX = [so3.apply(rot_matrixX, vectorops.sub(world_position_matrix[x],
                                                                        self.world.rigidObject(0).getTransform()[1]))
                                   for x
                                   in range(3)]
    ...
    def goal_rotation_transfer(self, robot, cspace, obj_pt):
        """
...
class GLPickAndPlacePlugin(GLPluginInterface):
    ...
    def keyboardfunc(self, key, x, y):
        h = 0.265
        targets = {'a': (0.5, 0.2, h), 'b': (0.5, -0.05, h), 'c': (-0.6, -0.05, h), 'd': (-0.6, 0.2, h)}
        if key in targets:
            dest = targets[key]
            ...
            if self.transitPath:
                ...
                if self.transferPath:
                    self.Tgoal = graspedObjectTransform(self.robot, self.hand, self.transferPath[0], self.Tstart,
                                                        self.transferPath[-1])
                    self.Tgoal_col_tmp = graspedObjectTransform(self.robot, self.hand, self.transferPath[0],
                                                                self.Tstart_col,
                                                                self.transferPath[-1])
                    self.Tgoal_col = (self.Tgoal_col_tmp[0], vectorops.add(self.Tgoal_col_tmp[1], [0, 0.45, 0]))
                    ...
                    print("Tgoal", *self.Tgoal)
                    print("Tgoal_col", *self.Tgoal_col)
                    print("Tgrsp", self.Tgrasp)
...
def planTransit(world, objectIndex, hand):
    ...
    qpregrasp, qpregrasparm = hand.ikSolver(robot, cspace, obj.getTransform()[1], False)
...
def planTransfer(world, objectIndex, hand, shift):
    ...
    qungrasp, qungrasparm = hand.ikSolver(robot, cspace,
                                          vectorops.add(vectorops.add(obj.getTransform()[1], [0, 0, 0.0]), shift), True)
