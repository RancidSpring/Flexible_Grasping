...
trans_ind = 0
filename = "grasping_database/cube_grasp_uti2.csv"
...
class Hand:
    ...
    def solve_transit(self, robot, cspace, obj_pt):
        global trans_ind
        global start_angle
        while trans_ind < len(self.heu_dict)-2:
            goal = ik.objective(robot.link("tool0"),
                                local=[self.localPosition1, self.localPosition2, self.localPosition3],
                                world=[vectorops.add(self.heu_dict[trans_ind][0], obj_pt), vectorops.add(self.heu_dict[trans_ind][1], obj_pt), vectorops.add(self.heu_dict[trans_ind+2][2], obj_pt)])
            start_angle = self.heu_dict[trans_ind+4]
            trans_ind += 5
        ...
        for obstacle in obs_arr:
            ...
            for i in range(1000):
                ...
                and delta_pitch < i*heuristic_radius:
                    ...
                    break
            value += level
        heuristic_values.append([lines[index-2], lines[index-3], lines[index-1], value])
        index += 4
