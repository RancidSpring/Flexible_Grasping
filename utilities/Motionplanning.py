...
from utilities.TransferUtility import TransferCSpace
from utilities.TransitUtility import TransitCSpace
...
def planTransit(world, objectIndex, hand):
    ...
    # if hand.gripper == "2fing_adaptive":
    for i in range(1, 101):
        qarm_tmp = path + [hand.open_free(path[-1], 1-(i/100))]
        final_qarm = qarm_tmp[-1]
        robot.setConfig(final_qarm)
        if any(globals.collider.robotObjectCollisions(robot.index, obj.index)):
            break

    # elif hand.gripper == "3fing_adaptive":
    #     contact_arr = [15, 16, 17, 18]
    #     for i in range(1, 101):
    #         print("CONTACT ARRAY", contact_arr)
    #         qarm_tmp = path + [hand.open(path[-1], contact_arr, 1-(i/100))]
    #         final_qarm = qarm_tmp[-1]
    #         robot.setConfig(final_qarm)
    #         if any(globals.collider.robotObjectCollisions(robot.index, obj.index)):
    #             contact_index = list(globals.collider.robotObjectCollisions(robot.index, obj.index))[0][0].index
    #             if contact_index not in contact_arr:
    #                 contact_arr.append(contact_index)
    #                 contact_arr.append(contact_index-1)
    #                 contact_arr.append(contact_index-2)
    #         if len(contact_arr) == len(hand.handIndices):
    #             break
...
def planTransfer(world, objectIndex, hand, shift):
    ...
    print("transfer IK solver time", end - start)
    ...
    qpostungrasp = hand.open_free(qungrasp, 1)
    end = time.time()
    print("Transfer motion planning time: ", end - start)
...
def planFree(world, hand, qtarget):
    ...
    for qarm in planner.getPath():
        ...
        for qi, i in zip(qarm, hand.armIndices):
            path[-1][i] = qi
    return path + [hand.open_free(path[-1], 1)]
